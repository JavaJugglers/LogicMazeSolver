...
def sentence1() -> Expr:
    ...
    A, B, C = map(Expr, ['A', 'B', 'C'])
    """
    Use conjoin & disjoin when there are more than 2 variables in the chained and / or.
    """
...
def sentence3() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    PacmanAlive = list(map(lambda t: PropSymbolExpr('PacmanAlive', time=t), [0, 1]))
    PacmanBorn = [PropSymbolExpr('PacmanBorn', time=0)]
    PacmanKilled = [PropSymbolExpr('PacmanKilled', time=0)]
    return conjoin([
        PacmanAlive[1] % (PacmanAlive[0] & ~PacmanKilled[0] | ~PacmanAlive[0] & PacmanBorn[0]),
        ~(PacmanAlive[0] & PacmanBorn[0]),
        PacmanBorn[0]
    ])
    "*** END YOUR CODE HERE ***"
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    """Returns True if the premise entails the conclusion and False otherwise.
       Entailment means that in every world where premise is true, conclusion is also true.
       So if there exists one world where premise is true, but conclusion is false, then return false.
...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    return not pl_true(inverse_statement, model=assignments)
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
        reference:
        https://baldur.iti.kit.edu/sat/files/2017/l02.pdf
    """

    # inversed_literals = list(map(lambda e: ~e, literals))
    # return conjoin(list(map(lambda ls: ls[0] | ls[1], itertools.combinations(inversed_literals, 2))))
    return conjoin(list(map(lambda ls: ~ls[0] | ~ls[1], itertools.combinations(literals, 2))))
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for location in all_coords:
        pacphysics_sentences.append(PropSymbolExpr(wall_str, location[0], location[1]) >> ~PropSymbolExpr(pacman_str, location[0], location[1], time=t))
    pacphysics_sentences.append(exactlyOne(
        list(map(lambda location: PropSymbolExpr(pacman_str, location[0], location[1], time=t), non_outer_wall_coords))
    ))
    pacphysics_sentences.append(exactlyOne(
        list(map(lambda directon: PropSymbolExpr(directon, time=t), DIRECTIONS))
    ))
    if sensorModel:
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    map_sent = [PropSymbolExpr(wall_str, x, y) if (x, y) in walls_set else ~PropSymbolExpr(wall_str, x, y) for x in range(problem.getWidth() + 2) for y in range(problem.getHeight() + 2)]
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        print("Step: {}".format(t))
        KB.append(exactlyOne(list(map(lambda l: PropSymbolExpr(pacman_str, l[0], l[1], time=t), non_wall_coords))))
        solution = findModel(conjoin(KB + [PropSymbolExpr(pacman_str, xg, yg, time=t)]))
        if solution:
            return extractActionSequence(solution, actions)
        KB.append(exactlyOne(list(map(lambda a: PropSymbolExpr(a, time=t), actions))))
        for location in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(location[0], location[1], t + 1, walls_grid))
    "*** END YOUR CODE HERE ***"
...
def foodSuccessorAxiom(x: int, y: int, time: int, walls_grid: List[List[bool]] = None) -> Expr:
    ...
    """
    now, last = time, time - 1
    possible_causes: List[Expr] = [~PropSymbolExpr(food_str, x, y, time=last)]  # enumerate all possible causes for P[x,y]_t
    # the if statements give a small performance boost and are required for q4 and q5 correctness
    if walls_grid[x][y + 1] != 1:
        possible_causes.append(PropSymbolExpr(pacman_str, x, y + 1, time=last)
                               & PropSymbolExpr('South', time=last))
    ...
    "*** BEGIN YOUR CODE HERE ***"
    return (~PropSymbolExpr(food_str, x, y, time=time)) % (disjoin(possible_causes))
    "*** END YOUR CODE HERE ***"
...
def foodLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
    KB += [PropSymbolExpr(food_str, x, y, time=0) if (x, y) in food_set else ~PropSymbolExpr(food_str, x, y, time=0) for x in range(width + 2) for y in range(height + 2)]
    for t in range(50):
        print("Step: {}".format(t))
        KB.append(exactlyOne(list(map(lambda l: PropSymbolExpr(pacman_str, l[0], l[1], time=t), non_wall_coords))))
        solution = findModel(conjoin(KB + list(map(lambda l: ~PropSymbolExpr(food_str, l[0], l[1], time=t), food))))
        if solution:
            return extractActionSequence(solution, actions)
        KB.append(exactlyOne(list(map(lambda a: PropSymbolExpr(a, time=t), actions))))
        for location in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(location[0], location[1], t + 1, walls_grid))
            KB.append(foodSuccessorAxiom(location[0], location[1], t + 1, walls_grid))
    "*** END YOUR CODE HERE ***"
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for (x, y) in non_outer_wall_coords:
            if known_map[x][y] != 1:
                must_at_x_y = entails(conjoin(KB), PropSymbolExpr(pacman_str, x, y, time=t))
                must_not_at_x_y = entails(conjoin(KB), ~PropSymbolExpr(pacman_str, x, y, time=t))
                assert not (must_at_x_y is True and must_not_at_x_y is True)
                if must_at_x_y:
                    KB.append(PropSymbolExpr(pacman_str, x, y, time=t))
                    possible_locations.append((x, y))
                elif must_not_at_x_y:
                    KB.append(~PropSymbolExpr(pacman_str, x, y, time=t))
                else:
                    possible_locations.append((x, y))
...
def mapping(problem, agent) -> Generator:
    ...
    must_be_wall = entails(conjoin(KB), PropSymbolExpr(wall_str, pac_x_0,pac_y_0))
    must_not_be_wall = entails(conjoin(KB), ~PropSymbolExpr(wall_str, pac_x_0, pac_y_0))
    if must_be_wall:
        known_map[x][y] = 1
        KB.append(PropSymbolExpr(wall_str, pac_x_0,pac_y_0))
    elif must_not_be_wall:
        known_map[x][y] = 0
        KB.append(~PropSymbolExpr(wall_str, pac_x_0,pac_y_0))
    ...
    for t in range(agent.num_timesteps):
        ...
        # find provable wall location
        for (x, y) in non_outer_wall_coords:
            if known_map[x][y] == -1:
                must_be_wall = entails(conjoin(KB), PropSymbolExpr(wall_str, x, y))
                must_not_be_wall = entails(conjoin(KB), ~PropSymbolExpr(wall_str, x, y))
                assert not (must_be_wall is True and must_not_be_wall is True)
                if must_be_wall:
                    known_map[x][y] = 1
                    KB.append(PropSymbolExpr(wall_str, x, y))
                elif must_not_be_wall:
